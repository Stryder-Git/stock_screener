...
public class ScreenerController : ControllerBase
{
        ...
        string[] GetSymbols(string configs)
        {
             ...
             return JsonConvert.DeserializeObject<Symbols>(strSyms).symbols;

        }

        [HttpGet]
        IConsumer<Ignore, string> GetConsumer(string c)
        {

            string[] symbols = GetSymbols(c);
        ...
        [HttpGet]
        public async Task Get(string c)
        {   sse_connected = true;
            this.sseid = ++id;
            Screeners[this.sseid] = this;
             

            IConsumer<Ignore, string> consumer = GetConsumer(c);
